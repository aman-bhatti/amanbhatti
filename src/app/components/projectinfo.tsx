import Link from "next/link";
import { Reveal } from "../../../lib/fade";
import { RxArrowTopRight } from "react-icons/rx";

// Define the tech stack with SVG paths
const tech_stack = [
  { name: "Typescript", icon: "/icons/typescript.svg" },
  { name: "Python", icon: "/icons/python.svg" },
  { name: "React.js", icon: "/icons/reactjs.svg" },
  { name: "Java", icon: "/icons/java.svg" },
  { name: "Golang", icon: "/icons/go.svg" },
  { name: "Javascript", icon: "/icons/javascript.svg" },
  { name: "Cryptography", icon: "/icons/encryption-icon.svg" },
  { name: "Machine Learning", icon: "/icons/ml.png" },
];

const tech_svg = [
  {
    name: "Golang",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="w-5 h-5"
          viewBox="0 0 45 45"
        >
          <path
            fill="#ffcc80"
            d="M35.547 42.431l-3.182-3.182-2.121 2.121 3.182 3.182c.586.586 1.536.586 2.121 0S36.133 43.017 35.547 42.431zM38.547 24.431l-3.182-3.182-2.121 2.121 3.182 3.182c.586.586 1.536.586 2.121 0S39.133 25.017 38.547 24.431zM12.683 42.431l3.182-3.182 2.121 2.121-3.182 3.182c-.586.586-1.536.586-2.121 0S12.097 43.017 12.683 42.431zM9.433 24.431l3.182-3.182 2.121 2.121-3.182 3.182c-.586.586-1.536.586-2.121 0S8.847 25.017 9.433 24.431z"
          ></path>
          <path
            fill="#4dd0e1"
            d="M38 8c0-1.933-1.149-3-3.231-3S31 7.567 31 9.5c0 1 1.923 1.5 3 1.5C36.082 11 38 9.933 38 8zM10 8c0-1.933 1.149-3 3.231-3S17 7.567 17 9.5c0 1-1.923 1.5-3 1.5C11.918 11 10 9.933 10 8z"
          ></path>
          <path
            fill="#424242"
            d="M35 7A1 1 0 1 0 35 9 1 1 0 1 0 35 7zM13 7A1 1 0 1 0 13 9 1 1 0 1 0 13 7z"
          ></path>
          <path
            fill="#4dd0e1"
            d="M37,34c0,4.774-3.219,10-13.31,10C15.568,44,11,38.774,11,34c0-5,1-5.806,1-10c0-4.688,0-7,0-10 c0-4.774,3.076-11,11.69-11S36,6.991,36,13c0,3-0.237,5.453,0,10C36.186,26.562,37,31,37,34z"
          ></path>
          <g>
            <path
              fill="#f5f5f5"
              d="M29 6A4 4 0 1 0 29 14 4 4 0 1 0 29 6zM19 6A4 4 0 1 0 19 14 4 4 0 1 0 19 6z"
            ></path>
          </g>
          <g>
            <path
              fill="#eee"
              d="M24 20c0 .552.448 1 1 1s1-.448 1-1v-3h-2V20zM22 20c0 .552.448 1 1 1s1-.448 1-1v-3h-2V20z"
            ></path>
          </g>
          <path
            fill="#ffcc80"
            d="M26.5,18c-0.412,0-0.653-0.085-1.011-0.205c-0.975-0.328-2.021-0.326-2.996,0.002 C22.138,17.916,21.91,18,21.5,18c-1.334,0-1.5-1-1.5-1.5c0-1.5,1.5-2.5,3-2.5c0.835,0,1.165,0,2,0c1.5,0,3,1,3,2.5 C28,17,27.834,18,26.5,18z"
          ></path>
          <g>
            <path
              fill="#424242"
              d="M27 9A1 1 0 1 0 27 11 1 1 0 1 0 27 9zM17 9A1 1 0 1 0 17 11 1 1 0 1 0 17 9zM24 13A2 1 0 1 0 24 15 2 1 0 1 0 24 13z"
            ></path>
          </g>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          Golang
        </span>
      </div>
    ),
  },
  {
    name: "Typescript",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 32 32"
          fill="var(--h-color)"
          className="w-5l h-5"
        >
          <path
            d="M0 16v16h32v-32h-32zM25.786 14.724c0.813 0.203 1.432 0.568 2.005 1.156 0.292 0.312 0.729 0.885 0.766 1.026 0.010 0.042-1.38 0.974-2.224 1.495-0.031 0.021-0.156-0.109-0.292-0.313-0.411-0.599-0.844-0.859-1.505-0.906-0.969-0.063-1.594 0.443-1.589 1.292-0.005 0.208 0.042 0.417 0.135 0.599 0.214 0.443 0.615 0.708 1.854 1.245 2.292 0.984 3.271 1.635 3.88 2.557 0.682 1.031 0.833 2.677 0.375 3.906-0.51 1.328-1.771 2.234-3.542 2.531-0.547 0.099-1.849 0.083-2.438-0.026-1.286-0.229-2.505-0.865-3.255-1.698-0.297-0.323-0.87-1.172-0.833-1.229 0.016-0.021 0.146-0.104 0.292-0.188s0.682-0.396 1.188-0.688l0.922-0.536 0.193 0.286c0.271 0.411 0.859 0.974 1.214 1.161 1.021 0.542 2.422 0.464 3.115-0.156 0.281-0.234 0.438-0.594 0.417-0.958 0-0.37-0.047-0.536-0.24-0.813-0.25-0.354-0.755-0.656-2.198-1.281-1.651-0.714-2.365-1.151-3.010-1.854-0.406-0.464-0.708-1.010-0.88-1.599-0.12-0.453-0.151-1.589-0.057-2.042 0.339-1.599 1.547-2.708 3.281-3.036 0.563-0.109 1.875-0.068 2.427 0.068zM18.276 16.063l0.010 1.307h-4.167v11.839h-2.948v-11.839h-4.161v-1.281c0-0.714 0.016-1.307 0.036-1.323 0.016-0.021 2.547-0.031 5.62-0.026l5.594 0.016z"
            fill="#1976D2"
          ></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          Typescript
        </span>
      </div>
    ),
  },
  {
    name: "Javascript",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="var(--h-color)"
          className="w-5 h-5"
        >
          <path
            d="M0 0H24V24H0V0ZM18.3467 20.1196C17.2343 20.1196 16.6053 19.5394 16.1219 18.7501L14.2891 19.8149C14.9512 21.123 16.3044 22.1211 18.3989 22.1211C20.5409 22.1211 22.136 21.0088 22.136 18.9783C22.136 17.0949 21.0541 16.2571 19.1379 15.4354L18.5741 15.1939C17.6065 14.7749 17.1874 14.501 17.1874 13.8244C17.1874 13.277 17.6061 12.8579 18.2667 12.8579C18.9143 12.8579 19.3314 13.131 19.7181 13.8244L21.4743 12.6968C20.7314 11.3901 19.7006 10.891 18.2667 10.891C16.2526 10.891 14.9638 12.1787 14.9638 13.8701C14.9638 15.7063 16.0449 16.5749 17.6724 17.2682L18.2362 17.5101C19.2648 17.96 19.8781 18.2339 19.8781 19.0072C19.8781 19.6526 19.2811 20.1196 18.3467 20.1196ZM9.60647 20.1055C8.83161 20.1055 8.50933 19.5741 8.15504 18.9455L6.31923 20.057C6.85104 21.1825 7.89676 22.117 9.70247 22.117C11.7009 22.117 13.0701 21.0541 13.0701 18.7189V11.0198H10.8149V18.6884C10.8149 19.8156 10.3474 20.1055 9.60647 20.1055Z"
            fill="#F7DF1E"
          ></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          Javascript
        </span>
      </div>
    ),
  },
  {
    name: "HTML",
    svg: (
      <div className="relative group">
        <svg
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
          fill="var(--h-color)"
          className="w-5 h-5"
        >
          <path d="M5.08057 0H6.1623V1.06875H7.15183V0H8.23361V3.23644H7.15188V2.15269H6.16235V3.23644H5.08061L5.08057 0Z"></path>
          <path d="M9.65622 1.07325H8.7039V0H11.6908V1.07325H10.738V3.23644H9.65626L9.65622 1.07325Z"></path>
          <path d="M12.1648 0H13.2928L13.9866 1.13719L14.6797 0H15.8081V3.23644H14.7308V1.63228L13.9866 2.78302H13.968L13.2232 1.63228V3.23644H12.1648V0Z"></path>
          <path d="M16.3465 0H17.4286V2.16666H18.9498V3.23644H16.3465V0Z"></path>
          <path
            d="M5.04593 22.0724L3 4.71658H21L18.9527 22.0697L11.9897 24L5.04593 22.0724ZM9.18344 12.5727L8.98891 10.3929H17.1343L17.3249 8.26428H6.6626L7.23672 14.7013L14.6138 14.7013L14.3668 17.4619L11.9927 18.1027L11.991 18.1034L9.62038 17.4633L9.46883 15.7656H7.33203L7.63025 19.1079L11.9906 20.3183L16.3565 19.1078L16.8887 13.1443L16.9406 12.5727L9.18344 12.5727Z"
            fill="#E65100"
          ></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          HTML
        </span>
      </div>
    ),
  },
  {
    name: "Cryptography",
    svg: (
      <svg
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
        fill="var(--h-color)"
        className="w-5 h-5"
      >
        <path
          d="M 12.5 0 C 12.367418 0.00011174 12.240289 0.052789295 12.146484 0.14648438 L 7.1464844 5.1464844 C 7.0527894 5.2402891 7.0001119 5.3674177 7 5.5 L 7 6.4277344 C 6.97868 6.5848472 7.033013 6.7427762 7.1464844 6.8535156 L 8.7929688 8.5 L 0.14648438 17.146484 C 0.001290365 17.275926 -0.05582365 17.477729 0 17.664062 L 0 19.5 C 2.7608953e-005 19.776131 0.22386906 19.999972 0.5 20 L 3.5 20 C 3.6325994 19.999975 3.7597597 19.94728 3.8535156 19.853516 L 4.7070312 19 L 6.5 19 C 6.7761309 18.999972 6.9999724 18.776131 7 18.5 L 7 17 L 8.5 17 C 8.7761309 16.999972 8.9999724 16.776131 9 16.5 L 9 14.707031 L 12 11.707031 L 13.146484 12.853516 C 13.275926 12.99871 13.47773 13.055824 13.664062 13 L 14.5 13 C 14.632582 12.999888 14.759711 12.94721 14.853516 12.853516 L 19.853516 7.8535156 C 19.947211 7.7597108 19.999888 7.6325822 20 7.5 L 20 6.5 C 19.999888 6.3674178 19.947211 6.2402892 19.853516 6.1464844 L 13.853516 0.14648438 C 13.759711 0.052789315 13.632582 0.00011174 13.5 0 L 12.5 0 z M 12.707031 1 L 13.292969 1 L 19 6.7070312 L 19 7.2929688 L 14.292969 12 L 13.707031 12 L 12.353516 10.646484 C 12.257804 10.550855 12.127472 10.498018 11.992188 10.5 C 11.862317 10.5022 11.738364 10.55468 11.646484 10.646484 L 8.1464844 14.146484 C 8.0012904 14.275926 7.9441764 14.47773 8 14.664062 L 8 16 L 6.5 16 C 6.2238691 16.000028 6.0000276 16.223869 6 16.5 L 6 18 L 4.5 18 C 4.3674006 18.000025 4.2402403 18.05272 4.1464844 18.146484 L 3.2929688 19 L 1 19 L 1 17.707031 L 9.8535156 8.8535156 C 10.048266 8.6580714 10.048266 8.3419286 9.8535156 8.1464844 L 8 6.2929688 L 8 5.7070312 L 12.707031 1 z M 14.5 3.5 C 13.401353 3.5 12.5 4.4013528 12.5 5.5 C 12.5 6.5986472 13.401353 7.5 14.5 7.5 C 15.598647 7.5 16.5 6.5986472 16.5 5.5 C 16.5 4.4013528 15.598647 3.5 14.5 3.5 z M 14.5 4.5 C 15.058207 4.5 15.5 4.9417929 15.5 5.5 C 15.5 6.0582071 15.058207 6.5 14.5 6.5 C 13.941793 6.5 13.5 6.0582071 13.5 5.5 C 13.5 4.9417929 13.941793 4.5 14.5 4.5 z "
          fill=""
        ></path>
      </svg>
    ),
  },
  {
    name: "Python",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className="w-5 h-5"
        >
          <path
            d="M11.9135 0C5.82079 4.36975e-07 6.20126 2.65599 6.20126 2.65599L6.20805 5.40757H12.0222V6.23372H3.89871C3.89871 6.23372 0 5.78926 0 11.969C-4.34699e-07 18.1488 3.40288 17.9296 3.40288 17.9296H5.43374V15.062C5.43374 15.062 5.32428 11.6413 8.78229 11.6413C12.2403 11.6413 14.5489 11.6413 14.5489 11.6413C14.5489 11.6413 17.7887 11.6939 17.7887 8.4937C17.7887 5.29347 17.7887 3.20221 17.7887 3.20221C17.7887 3.20221 18.2806 0 11.9135 0ZM8.70758 1.85032C9.28604 1.85032 9.75357 2.3203 9.75357 2.90179C9.75357 3.48328 9.28604 3.95326 8.70758 3.95326C8.12912 3.95326 7.66158 3.48328 7.66158 2.90179C7.66158 2.3203 8.12912 1.85032 8.70758 1.85032Z"
            fill="#4584b6"
          ></path>
          <path
            d="M12.0867 24C18.1794 24 17.7989 21.344 17.7989 21.344L17.7921 18.5924H11.978V17.7663H20.1014C20.1014 17.7663 24.0002 18.2108 24.0002 12.031C24.0002 5.85123 20.5973 6.07038 20.5973 6.07038H18.5664V8.93803C18.5664 8.93803 18.6759 12.3587 15.2179 12.3587C11.7598 12.3587 9.4513 12.3587 9.4513 12.3587C9.4513 12.3587 6.21143 12.3061 6.21143 15.5063C6.21143 18.7065 6.21143 20.7978 6.21143 20.7978C6.21143 20.7978 5.71953 24 12.0867 24ZM15.2926 22.1497C14.7141 22.1497 14.2466 21.6797 14.2466 21.0982C14.2466 20.5167 14.7141 20.0468 15.2926 20.0468C15.871 20.0468 16.3386 20.5167 16.3386 21.0982C16.3386 21.6797 15.871 22.1497 15.2926 22.1497Z"
            fill="#ffde57"
          ></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          Python
        </span>
      </div>
    ),
  },
  {
    name: "vitejs",
    svg: (
      <div className="relative group">
        <svg
          viewBox="0 0 250 250"
          xmlns="http://www.w3.org/2000/svg"
          className="w-5 h-5"
        >
          <defs>
            <linearGradient
              x1="-.828%"
              y1="7.652%"
              x2="57.636%"
              y2="78.411%"
              id="a"
            >
              <stop stopColor="#41D1FF" offset="0%" />
              <stop stopColor="#BD34FE" offset="100%" />
            </linearGradient>
            <linearGradient
              x1="43.376%"
              y1="2.242%"
              x2="50.316%"
              y2="89.03%"
              id="b"
            >
              <stop stopColor="#FFEA83" offset="0%" />
              <stop stopColor="#FFDD35" offset="8.333%" />
              <stop stopColor="#FFA800" offset="100%" />
            </linearGradient>
          </defs>
          <path
            d="M255.153 37.938 134.897 252.976c-2.483 4.44-8.862 4.466-11.382.048L.875 37.958c-2.746-4.814 1.371-10.646 6.827-9.67l120.385 21.517a6.537 6.537 0 0 0 2.322-.004l117.867-21.483c5.438-.991 9.574 4.796 6.877 9.62Z"
            fill="url(#a)"
          />
          <path
            d="M185.432.063 96.44 17.501a3.268 3.268 0 0 0-2.634 3.014l-5.474 92.456a3.268 3.268 0 0 0 3.997 3.378l24.777-5.718c2.318-.535 4.413 1.507 3.936 3.838l-7.361 36.047c-.495 2.426 1.782 4.5 4.151 3.78l15.304-4.649c2.372-.72 4.652 1.36 4.15 3.788l-11.698 56.621c-.732 3.542 3.979 5.473 5.943 2.437l1.313-2.028 72.516-144.72c1.215-2.423-.88-5.186-3.54-4.672l-25.505 4.922c-2.396.462-4.435-1.77-3.759-4.114l16.646-57.705c.677-2.35-1.37-4.583-3.769-4.113Z"
            fill="url(#b)"
          />
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          Vite.js
        </span>
      </div>
    ),
  },
  {
    name: "Next.js",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 15 15"
          fill="var(--h-color)"
          className="w-5 h-5"
        >
          <path
            fillRule="evenodd"
            d="M0 7.5C0 3.35786 3.35786 0 7.5 0C11.6421 0 15 3.35786 15 7.5C15 10.087 13.6902 12.3681 11.6975 13.7163L4.90687 4.20942C4.78053 4.03255 4.5544 3.95756 4.34741 4.02389C4.14042 4.09022 4 4.28268 4 4.50004V12H5V6.06027L10.8299 14.2221C9.82661 14.7201 8.696 15 7.5 15C3.35786 15 0 11.6421 0 7.5ZM10 10V4H11V10H10Z"
          ></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          NextJS
        </span>
      </div>
    ),
  },
  {
    name: "React.js",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="1 0 28 28"
          fill="#00d8ff"
          className="w-5 h-5"
        >
          <path d="M16,21.706a28.385,28.385,0,0,1-8.88-1.2,11.3,11.3,0,0,1-3.657-1.958A3.543,3.543,0,0,1,2,15.974c0-1.653,1.816-3.273,4.858-4.333A28.755,28.755,0,0,1,16,10.293a28.674,28.674,0,0,1,9.022,1.324,11.376,11.376,0,0,1,3.538,1.866A3.391,3.391,0,0,1,30,15.974c0,1.718-2.03,3.459-5.3,4.541A28.8,28.8,0,0,1,16,21.706Zm0-10.217a27.948,27.948,0,0,0-8.749,1.282c-2.8.977-4.055,2.313-4.055,3.2,0,.928,1.349,2.387,4.311,3.4A27.21,27.21,0,0,0,16,20.51a27.6,27.6,0,0,0,8.325-1.13C27.4,18.361,28.8,16.9,28.8,15.974a2.327,2.327,0,0,0-1.01-1.573,10.194,10.194,0,0,0-3.161-1.654A27.462,27.462,0,0,0,16,11.489Z fill:#00d8ff"></path>
          <path d="M10.32,28.443a2.639,2.639,0,0,1-1.336-.328c-1.432-.826-1.928-3.208-1.327-6.373a28.755,28.755,0,0,1,3.4-8.593h0A28.676,28.676,0,0,1,16.71,5.995a11.376,11.376,0,0,1,3.384-2.133,3.391,3.391,0,0,1,2.878,0c1.489.858,1.982,3.486,1.287,6.859a28.806,28.806,0,0,1-3.316,8.133,28.385,28.385,0,0,1-5.476,7.093,11.3,11.3,0,0,1-3.523,2.189A4.926,4.926,0,0,1,10.32,28.443Zm1.773-14.7a27.948,27.948,0,0,0-3.26,8.219c-.553,2.915-.022,4.668.75,5.114.8.463,2.742.024,5.1-2.036a27.209,27.209,0,0,0,5.227-6.79,27.6,27.6,0,0,0,3.181-7.776c.654-3.175.089-5.119-.713-5.581a2.327,2.327,0,0,0-1.868.089A10.194,10.194,0,0,0,17.5,6.9a27.464,27.464,0,0,0-5.4,6.849Z fill:#00d8ff"></path>
          <path d="M21.677,28.456c-1.355,0-3.076-.82-4.868-2.361a28.756,28.756,0,0,1-5.747-7.237h0a28.676,28.676,0,0,1-3.374-8.471,11.376,11.376,0,0,1-.158-4A3.391,3.391,0,0,1,8.964,3.9c1.487-.861,4.01.024,6.585,2.31a28.8,28.8,0,0,1,5.39,6.934,28.384,28.384,0,0,1,3.41,8.287,11.3,11.3,0,0,1,.137,4.146,3.543,3.543,0,0,1-1.494,2.555A2.59,2.59,0,0,1,21.677,28.456Zm-9.58-10.2a27.949,27.949,0,0,0,5.492,6.929c2.249,1.935,4.033,2.351,4.8,1.9.8-.465,1.39-2.363.782-5.434A27.212,27.212,0,0,0,19.9,13.74,27.6,27.6,0,0,0,14.755,7.1c-2.424-2.152-4.39-2.633-5.191-2.169a2.327,2.327,0,0,0-.855,1.662,10.194,10.194,0,0,0,.153,3.565,27.465,27.465,0,0,0,3.236,8.1Z fill:#00d8ff"></path>
        </svg>
        <span className="absolute left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 whitespace-nowrap group-hover:opacity-100 transition-opacity duration-300">
          ReactJS
        </span>
      </div>
    ),
  },
  {
    name: "Machine Learning",
    svg: (
      <div className="relative group">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 30 30"
          fill="var(--h-color)"
          className="w-5 h-5"
        >
          <path
            id="machine--learning--03_1_"
            d="M29.36,15.695v-3.389c0.575-0.159,1-0.681,1-1.305c0-0.75-0.61-1.36-1.36-1.36
	c-0.191,0-0.372,0.04-0.537,0.111l-2.365-2.956C26.261,6.572,26.36,6.298,26.36,6c0-0.75-0.61-1.36-1.36-1.36
	c-0.401,0-0.758,0.177-1.007,0.454l-3.659-1.83C20.351,3.179,20.36,3.091,20.36,3c0-0.75-0.61-1.36-1.36-1.36
	c-0.625,0-1.147,0.425-1.306,1h-4.389c-0.159-0.575-0.681-1-1.305-1c-0.75,0-1.36,0.61-1.36,1.36c0,0.091,0.01,0.179,0.027,0.264
	l-2.66,1.33C7.758,4.318,7.401,4.14,7,4.14c-0.75,0-1.36,0.61-1.36,1.36c0,0.273,0.082,0.526,0.221,0.739L3.598,8.785
	C3.417,8.695,3.216,8.64,3,8.64c-0.75,0-1.36,0.61-1.36,1.36c0,0.625,0.425,1.146,1,1.305v3.389c-0.575,0.159-1,0.681-1,1.305
	c0,0.75,0.61,1.36,1.36,1.36c0.546,0,1.014-0.325,1.231-0.791l3.424,0.571C7.726,17.824,8.298,18.36,9,18.36
	c0.091,0,0.179-0.01,0.264-0.027l0.83,1.659C9.818,20.242,9.64,20.599,9.64,21c0,0.75,0.61,1.36,1.36,1.36
	c0.47,0,0.885-0.24,1.13-0.604l3.518,1.173C15.646,22.953,15.64,22.976,15.64,23c0,0.625,0.425,1.147,1,1.306v3.388
	c-0.575,0.159-1,0.681-1,1.306c0,0.75,0.61,1.36,1.36,1.36s1.36-0.61,1.36-1.36c0-0.624-0.425-1.147-1-1.306v-3.389
	c0.575-0.159,1-0.681,1-1.306c0-0.452-0.224-0.85-0.564-1.098l0.682-1.82l3.163,0.904c0,0.005-0.001,0.01-0.001,0.015
	c0,0.624,0.425,1.147,1,1.306v5.389c-0.575,0.159-1,0.681-1,1.306c0,0.75,0.61,1.36,1.36,1.36s1.36-0.61,1.36-1.36
	c0-0.624-0.425-1.147-1-1.306v-5.389c0.575-0.159,1-0.681,1-1.306c0-0.149-0.03-0.29-0.074-0.424l3.828-2.553
	C28.353,18.231,28.66,18.36,29,18.36c0.75,0,1.36-0.61,1.36-1.36C30.36,16.376,29.935,15.854,29.36,15.695z M21.354,14.932
	c-0.002-0.045-0.007-0.09-0.014-0.134l3.049-1.355l3.313,2.899L21.354,14.932z M20,15.64c-0.353,0-0.64-0.287-0.64-0.64
	s0.287-0.64,0.64-0.64s0.64,0.287,0.64,0.64S20.353,15.64,20,15.64z M20,13.64c-0.75,0-1.36,0.61-1.36,1.36
	c0,0.452,0.224,0.85,0.564,1.097l-1.166,3.11l-1.902-0.543l-1.465-5.126l5.463-3.756l0.628,0.489l3.028,2.65l-2.745,1.22
	C20.796,13.838,20.423,13.64,20,13.64z M9.131,15.653L8.122,9.85l3.612-0.361V9.485C11.93,9.995,12.421,10.36,13,10.36
	c0.005,0,0.009-0.001,0.014-0.001l0.823,2.879l-3.966,2.726C9.666,15.792,9.412,15.681,9.131,15.653z M13,9.64
	c-0.353,0-0.64-0.287-0.64-0.64S12.647,8.36,13,8.36S13.64,8.647,13.64,9S13.353,9.64,13,9.64z M10.28,16.557l3.765-2.589
	l1.275,4.463l-4.962-1.418c0-0.005,0.001-0.009,0.001-0.014C10.36,16.844,10.328,16.697,10.28,16.557z M24.129,7.036
	C24.366,7.235,24.667,7.36,25,7.36c0.191,0,0.372-0.04,0.537-0.111l2.365,2.955C27.739,10.428,27.64,10.702,27.64,11
	c0,0.069,0.01,0.135,0.02,0.201l-3.144,1.397l-3.295-2.883l-0.464-0.361L24.129,7.036z M28.251,15.866l-3.138-2.745l2.841-1.262
	c0.176,0.213,0.412,0.371,0.686,0.446v3.389C28.501,15.733,28.369,15.789,28.251,15.866z M29.64,11c0,0.353-0.287,0.64-0.64,0.64
	s-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64S29.64,10.647,29.64,11z M25,5.36c0.353,0,0.64,0.287,0.64,0.64S25.353,6.64,25,6.64
	S24.36,6.353,24.36,6S24.647,5.36,25,5.36z M19,2.36c0.353,0,0.64,0.287,0.64,0.64S19.353,3.64,19,3.64S18.36,3.353,18.36,3
	S18.647,2.36,19,2.36z M19,4.36c0.401,0,0.758-0.177,1.007-0.454l3.659,1.83C23.649,5.821,23.64,5.909,23.64,6
	c0,0.156,0.032,0.303,0.08,0.443l-3.56,2.448l-6.904-5.37c0.021-0.051,0.035-0.106,0.05-0.161h4.389
	C17.853,3.935,18.375,4.36,19,4.36z M12,2.36c0.353,0,0.64,0.287,0.64,0.64S12.353,3.64,12,3.64S11.36,3.353,11.36,3
	S11.647,2.36,12,2.36z M10.993,3.906C11.242,4.182,11.599,4.36,12,4.36c0.304,0,0.583-0.104,0.81-0.274l6.728,5.232l-5.075,3.489
	l-0.757-2.65C14.096,9.918,14.36,9.491,14.36,9c0-0.75-0.61-1.36-1.36-1.36c-0.673,0-1.229,0.492-1.337,1.135V8.772L7.998,9.138
	l-0.42-2.412C8.039,6.508,8.36,6.042,8.36,5.5c0-0.091-0.01-0.179-0.027-0.264L10.993,3.906z M4.265,9.511
	C4.231,9.423,4.19,9.339,4.139,9.261l2.263-2.546c0.143,0.071,0.3,0.115,0.467,0.132L7.28,9.21L4.265,9.511z M7,4.86
	c0.353,0,0.64,0.287,0.64,0.64S7.353,6.14,7,6.14S6.36,5.853,6.36,5.5S6.647,4.86,7,4.86z M2.36,10c0-0.353,0.287-0.64,0.64-0.64
	S3.64,9.647,3.64,10S3.353,10.64,3,10.64S2.36,10.353,2.36,10z M3,16.64c-0.353,0-0.64-0.287-0.64-0.64S2.647,15.36,3,15.36
	S3.64,15.647,3.64,16C3.64,16.353,3.353,16.64,3,16.64z M7.769,16.43l-3.424-0.571c-0.059-0.561-0.455-1.018-0.986-1.165v-3.389
	c0.503-0.139,0.888-0.556,0.977-1.077l3.067-0.307l1.018,5.853C8.135,15.91,7.904,16.142,7.769,16.43z M8.36,17
	c0-0.353,0.287-0.64,0.64-0.64S9.64,16.647,9.64,17S9.353,17.64,9,17.64S8.36,17.353,8.36,17z M11,21.64
	c-0.353,0-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64s0.64,0.287,0.64,0.64S11.353,21.64,11,21.64z M12.353,21.071
	c0.001-0.024,0.007-0.047,0.007-0.071c0-0.75-0.61-1.36-1.36-1.36c-0.091,0-0.179,0.01-0.264,0.027l-0.83-1.659
	c0.098-0.088,0.182-0.189,0.251-0.302l5.396,1.541l0.742,2.596c-0.168,0.103-0.315,0.238-0.425,0.402L12.353,21.071z M17.64,29
	c0,0.353-0.287,0.64-0.64,0.64c-0.353,0-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64C17.353,28.36,17.64,28.647,17.64,29z M17,23.64
	c-0.353,0-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64c0.353,0,0.64,0.287,0.64,0.64S17.353,23.64,17,23.64z M17.121,21.652
	C17.08,21.648,17.041,21.64,17,21.64c-0.005,0-0.009,0.001-0.014,0.001l-0.618-2.162l1.416,0.404L17.121,21.652z M18.732,19.406
	l1.147-3.059c0.041,0.004,0.08,0.012,0.121,0.012c0.519,0,0.967-0.296,1.196-0.725l6.451,1.433c0.006,0.124,0.03,0.243,0.068,0.356
	l-3.828,2.553C23.647,19.769,23.34,19.64,23,19.64c-0.491,0-0.918,0.264-1.158,0.655L18.732,19.406z M23.64,29
	c0,0.353-0.287,0.64-0.64,0.64s-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64S23.64,28.647,23.64,29z M23,21.64
	c-0.353,0-0.64-0.287-0.64-0.64s0.287-0.64,0.64-0.64s0.64,0.287,0.64,0.64S23.353,21.64,23,21.64z M29,17.64
	c-0.353,0-0.64-0.287-0.64-0.64c0-0.353,0.287-0.64,0.64-0.64s0.64,0.287,0.64,0.64C29.64,17.353,29.353,17.64,29,17.64z"
          ></path>
        </svg>
        <span className="absolute whitespace-nowrap left-0 top-8 bg-gray-200 text-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          Machine Learning
        </span>
      </div>
    ),
  },
];

interface ProjectCardProps {
  url: string;
  title: string;
  description: string;
  tech: string[]; // Pass the tech stack for each project
}

export default function ProjectCard({
  url,
  title,
  description,
  tech,
}: ProjectCardProps) {
  return (
    <Reveal>
      <div className="rounded-lg hover:shadow-lg transition duration-300">
        <Link
          href={`/proj/${url}`}
          className="flex items-center justify-between group"
        >
          <span className="text-lg font-medium group-hover:underline">
            {title}
          </span>
          <RxArrowTopRight className="text-neutral-500 ml-2 w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition duration-100" />
        </Link>
        <p className="proj-desc text-neutral-500">{description}</p>
        {/* Render tech stack icons */}
        <div className="flex space-x-2 mt-2">
          {tech.map((techName) => {
            const techItem = tech_svg.find((t) => t.name === techName);
            return techItem && <div key={techItem.name}>{techItem.svg}</div>;
          })}
        </div>
      </div>
    </Reveal>
  );
}
